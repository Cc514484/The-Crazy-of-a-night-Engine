...
{
	#if (cpp && windows)
	private static function applyNativeDarkModeEarly():Void
	{
		untyped __cpp__('
	...
	// กล่องรับขอบเขต UI ของ Preloader แยกต่างหาก
	...
	public function new()
	{
		...
		#if (cpp && windows)
		haxe.Timer.delay(applyNativeDarkModeEarly, 10);
		#end
	}
	...
	override function create():Void
	{
		...
		#if desktop
		Lib.current.stage.window.resize(1280, 720);
		#end
		...
		progressLines.graphics.lineStyle(2, 0xFF00CCFF);
		progressLines.graphics.drawRect(-2, this._height - BAR_PADDING - BAR_HEIGHT - 208, this._width + 4, 30);
		container.addChild(progressLines);

		var progressBarPiece = new Sprite();
		progressBarPiece.graphics.beginFill(0xFF00CCFF);
		...
		for (i in 0...amountOfPieces)
		{
			...
			piece.graphics.beginFill(0xFF00CCFF);
		...
		var progressLeftTextFormat = new TextFormat("DS-Digital", 32, 0xFF00CCFF, true);
		...
		container.addChild(progressLeftText);

		progressRightText = new TextField();

		var progressRightTextFormat = new TextFormat("DS-Digital", 16, 0xFF00CCFF, true);
		...
		container.addChild(progressRightText);

		box = new Sprite();
		box.graphics.beginFill(0xFF00CCFF, 1);
		box.graphics.drawRoundRect(0, 0, 64, 20, 5, 5);
		box.graphics.drawRoundRect(70, 0, 58, 20, 5, 5);
		box.graphics.endFill();
		box.graphics.beginFill(0xFF00CCFF, 0.1);
		...
		enhancedText.textColor = 0xFF00CCFF;
		...
		stereoText.textColor = 0xFF00CCFF;
		...
		vfdBitmap = new Bitmap(new BitmapData(Std.int(this._width), Std.int(this._height), true, 0xFFFFFFFF));
		container.addChild(vfdBitmap);
		...
		container.addChild(touchHereSprite);
		#end
	...
	{
		if (Lib.current == null || Lib.current.stage == null || container == null) return;
		
		var stageW:Float = Lib.current.stage.stageWidth;
		var stageH:Float = Lib.current.stage.stageHeight;
		
		var scale:Float = Math.min(stageW / 1280.0, stageH / 720.0);
		
		container.scaleX = scale;
		container.scaleY = scale;
		container.x = (stageW - (1280.0 * scale)) / 2;
		container.y = (stageH - (720.0 * scale)) / 2;
	}
		...
		switch (currentState)
		{
			...
			case FunkinPreloaderState.TouchHereToPlay:
				...
				if (touchHereToPlay.alpha < 1.0)
				{
					...
					if (vfdBitmap != null && container.contains(vfdBitmap))
						container.removeChild(vfdBitmap);
	...
	function updateProgressLeftText(text:Null<String>):Void
	{
		if (progressLeftText != null)
		{
			...
			else if (progressLeftText.text != text)
			{
				...
				enhancedText.defaultTextFormat = new TextFormat("Inconsolata Black", 16, 0xFF00CCFF, false);
				enhancedText.text = 'ENHANCED';
				enhancedText.textColor = 0xFF00CCFF;

				stereoText.defaultTextFormat = new TextFormat("Inconsolata Bold", 36, 0xFF00CCFF, false);
	...
	{
		Lib.current.stage.removeEventListener(Event.RESIZE, onWindowResize);
		_loaded = true;
	}
	...
	{
		if (logo != null && container != null && container.contains(logo))
