...
class DiscordClient
{
	...
	private inline static final _defaultID:String = "1542569594517065860"; // Application ID ของคุณ
	...
	private static function onReady(request:cpp.RawConstPointer<DiscordUser>):Void
	{
		...
		if (discriminator != '0') message += '($user#$discriminator)';
		else message += '($user)';
	...
	/**
	 * ฟังก์ชันสำหรับแสดงสถานะ "กำลังเล่นเพลง" โดยเฉพาะ
	 * @param songName ชื่อเพลง
	 * @param difficulty ระดับความยาก
	 * @param character ชื่อตัวละคร (จะถูกเติม 'face-' ข้างหน้าโดยอัตโนมัติ)
	 */
	public static function setPlayingPresence(songName:String, difficulty:String, character:String)
	{
		changePresence(
	...
	public static function changePresence(details:String = 'In the Menus', ?state:String, ?smallImageKey:String, ?hasStartTimestamp:Bool, ?endTimestamp:Float, largeImageKey:String = 'icon', ?largeImageText:String)
	{
		...
		if (largeImageText == null) largeImageText = 'P-Slice Engine';
